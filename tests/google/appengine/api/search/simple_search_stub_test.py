...
class SimpleSearchStubTest(absltest.TestCase):
  ...
  def testListIndexesNamespaceIsolation(self):
    self._IndexDocument('a', 'testid', [])
    self._IndexDocument('b', 'testid', [])
    self._IndexDocument('x', 'testid', [], namespace='ns1')
    self._IndexDocument('y', 'testid', [], namespace='ns2')

    self.CheckListIndexesResponse(self._ListIndexes(), ['a', 'b'],
                                  namespace_list=['', ''])
    self.CheckListIndexesResponse(self._ListIndexes(namespace='ns1'), ['x'],
                                  namespace_list=['ns1'])
    self.CheckListIndexesResponse(self._ListIndexes(namespace='ns2'), ['y'],
                                  namespace_list=['ns2'])

  def testListIndexesAllNamespaces(self):
    self._IndexDocument('a', 'testid', [])
    self._IndexDocument('b', 'testid', [])
    self._IndexDocument('x', 'testid', [], namespace='ns1')
    self._IndexDocument('y', 'testid', [], namespace='ns2')

    self.CheckListIndexesResponse(self._ListIndexes(all_namespaces=True),
                                  ['a', 'b', 'x', 'y'],
                                  namespace_list=['', '', 'ns1', 'ns2'])
